...
export default function PayrollOperations() {
  ...
  const selectedTeamFooter = weeklyPayrollTeamFooter(selectedTeam?.lines || [])
  ...
  const footerAmounts = (key) => positiveWeeklyPayrollFooterAmounts(selectedTeamFooter, key).map(([currency, amount]) => <span key={currency} className="block" dir="ltr">{money(amount, currency)}</span>)
  const transportFooterAmounts = footerAmounts('transport')
  ...
  const allTeamsHeaders = [t('common.team'), t('payroll.workers'), t('payroll.presentDays'), t('payroll.halfDays'), t('payroll.absentDays'), t('payroll.attendanceWage'), t('payroll.overtime'), t('payroll.finalTeamPayrollTotal')]
  const allTeamsRows = teamGroups.map((group) => [group.name, group.totals.workers, group.totals.presentDays, group.totals.halfDays, group.totals.absentDays, numeric(group.totals.baseAmount), numeric(group.totals.overtimeAmount), numeric(group.totals.finalAmount)])
  const allTeamsTotals = [[t('payroll.allTeamsPayrollTotal'), totals.workers, totals.presentDays, totals.halfDays, totals.absentDays, numeric(totals.baseAmount), numeric(totals.overtimeAmount), numeric(totals.finalAmount)]]
