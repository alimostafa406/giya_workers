...
export default function WeeklyPayrollTeamSummary({ groups, selectedTeamId = '', onSelectTeam }) {
  ...
  const summary = useMemo(() => weeklyPayrollTeamSummary(groups), [groups])
  ...
  return <><div className="weekly-team-summary-wrap"><table className="weekly-team-summary"><thead><tr><th>{t('payroll.summaryNumber')}</th><th className="summary-team">{t('payroll.summaryTeam')}</th><th>{t('payroll.summaryEffectif')}</th><th>{t('payroll.summaryWorkDayPay')}</th><th>{t('payroll.transport')}</th><th>{t('payroll.summaryOvertime')}</th><th>{t('payroll.summaryLate')}</th><th>{t('payroll.total')}</th></tr></thead><tbody>{summary.rows.map((row) => <tr key={row.id}><td>{row.index}</td><td className="summary-team"><span className="font-extrabold">{row.name}</span></td><td>{row.workers}</td><td>{values(row.byCurrency, 'workDayPay')}</td><td>{values(row.byCurrency, 'transport', true)}</td><td>{values(row.byCurrency, 'overtime', true)}</td><td>—</td><td className="summary-final">{values(row.byCurrency, 'total', true)}</td></tr>)}</tbody><tfoot><tr><th colSpan="2">{t('payroll.total')}</th><th>{summary.totals.workers}</th><th>{values(summary.totals.byCurrency, 'workDayPay')}</th><th>{values(summary.totals.byCurrency, 'transport', true)}</th><th>{values(summary.totals.byCurrency, 'overtime', true)}</th><th>—</th><th>{values(summary.totals.byCurrency, 'total')}</th></tr></tfoot></table></div>{!selectedTeamId ? <div className="mt-4"><PayrollTeamCards groups={groups} onOpenTeam={onSelectTeam} /></div> : null}</>
}
