...
export default function DailyOperationalReports({ type }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [attendanceResult, workersResult, mappingsResult] = await Promise.all([
    ...
    } catch (requestError) { setError(getErrorMessage(requestError)); setAttendance([]); setWorkers([]); setMappings([]) }
  ...
  const rows = useMemo(() => (overtime
    ...
    : buildDailyAttendanceExceptions({ workers, attendance, mappings, date: selectedDate })), [attendance, mappings, overtime, selectedDate, workers])
