...
export default function WorkerControlCenter() {
  ...
  const [state, setState] = useState({ workers: [], attendance: [], events: [], activated: [] })
  ...
  const details = { label: 'التفاصيل', render: r => <button className="btn-secondary px-3 py-1" onClick={() => setSelected(r)}>Details</button> }
  ...
  return <section className="w-full pb-12" dir="rtl">
    ...
    <ReportSection id="consecutive-absence" title={text.consecutive} rows={consecutive} columns={[worker, team, { label: 'الأيام المتتالية', render: r => r.longest || 0 }, { label: 'الخطورة', render: r => r.severity }, { label: 'غياب الأسبوع', render: r => r.weekAbsent || 0 }, { label: 'غياب الشهر', render: r => r.monthAbsent || 0 }, details]} />
    <ReportSection id="weekly-absence" title="مراقبة الغياب الأسبوعي" rows={weekly} columns={[worker, { label: 'كود الموظف', render: r => r.worker?.employee_code || '—' }, team, { label: 'غياب الأسبوع', render: r => r.weekAbsent || 0 }, { label: 'الغياب المتتالي', render: r => r.longest || 0 }, { label: 'غياب الشهر', render: r => r.monthAbsent || 0 }, { label: 'الخطورة', render: r => r.severity }, details]} />
